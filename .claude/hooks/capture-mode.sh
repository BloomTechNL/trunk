...
# ── /red "testName" ──────────────────────────────────────────────────
if echo "$PROMPT" | grep -qE '^/red[[:space:]]+"[^"]+"'; then
    TARGET=$(echo "$PROMPT" | sed -nE 's|^/red[[:space:]]+"([^"]+)".*|\1|p')
...
# ── /green ───────────────────────────────────────────────────────────
elif echo "$PROMPT" | grep -qE '^/green([[:space:]]|$)'; then
    cat > "$STATE_FILE" << STATE_EOF
MODE=green
STATE_EOF
...
# ── /refactor ────────────────────────────────────────────────────────
elif echo "$PROMPT" | grep -qE '^/refactor([[:space:]]|$)'; then
    cat > "$STATE_FILE" << STATE_EOF
MODE=refactor
STATE_EOF
